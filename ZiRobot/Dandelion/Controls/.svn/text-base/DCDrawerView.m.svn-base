...
-(DCDirectionDecision) decisionFromDirection:(DCMoveDirection) direction touch:(UITouch*) touch xDirection:(int)xDirection yDirection:(int)yDirection {
    ...
    if (_left.isDragEnabled && (_drawerLocation == DCDrawerLocationLeft || _drawerLocation == DCDrawerLocationLeftAndRight)) {
        if ((isExpanded && direction == DCMoveDirectionLeft) || (!isExpanded && x <= _left.handleWidth && xDirection == 1)) {
    ...
    if (_right.isDragEnabled && (_drawerLocation == DCDrawerLocationRight || _drawerLocation == DCDrawerLocationLeftAndRight)) {
        if ((isExpanded && direction == DCMoveDirectionRight) || (x >= self.frame.size.width - _right.handleWidth && xDirection == -1)) {
            return DCDirectionDecisionSieze;
        }
    }
...
-(void) directionTouchesBegan:(UITouch *)touch {
    ...
    if([self.delegate respondsToSelector:@selector(drawerViewIsFirstResponder)])
        [self.delegate drawerViewIsFirstResponder];
}
...
-(void) animateOffsetTo:(float) targetOffset {
    ...
    if (delegate) {
        if (_isLeftDrawerExpanded || _isRightDrawerExpanded) {
            if ([delegate respondsToSelector:@selector(drawerViewWillCollapse:)]) {
                [delegate drawerViewWillCollapse:self];
            }
        }
        else {
            if ([delegate respondsToSelector:@selector(drawerViewWillExpand:)]) {
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        if (drawnOutScale < 1) {
            ...
            if (delegate) {
                if (_isLeftDrawerExpanded || _isRightDrawerExpanded) {
                    if ([delegate respondsToSelector:@selector(drawerViewDidExpand:)]) {
                        [delegate drawerViewDidExpand:self];
                    }
                }
                else {
                    if ([delegate respondsToSelector:@selector(drawerViewDidCollapse:)]) {
