...
-(void) taskDidComplete: (DCTask*) task {
    ...
    if ([[task class] isSubclassOfClass:[DCFileDownloadTask class]]) {
        ...
        if (task.isSucceeded) {
            ...
            if (enableAppDelegate) {
                id <DCAppDelegate> delegate = [self findTargetWithSelector:@selector(appDidDownloadUrl:fileName:limitSize:)];
                if (delegate) {
                    [delegate appDidDownloadUrl:downloadTask.url fileName:downloadTask.fileName limitSize:downloadTask.limitSize];
                }
            }
        }
        else {
