...
+(void) dispatchDidDownloadUrl: (NSString*) url fileName:(NSString*) fileName limitSize:(int) limitSize {
    
    id <DCAppDelegate> target = [DCAppEventDispatcher findTargetWithSelector:@selector(appDidDownloadUrl:fileName:limitSize:)];
    if (target) {
        [target appDidDownloadUrl:url fileName:fileName limitSize:limitSize];
    }
}
