...
-(void) send:(id) callback {
    ...
    if (_serviceMethod.isExclusive && [RequestTracker isWaitingOnRequest]&&_serviceMethod.priority) {
        return;
    }
...
-(void) submitDownloadTask:(NSString*) url {
    ...
    if (_serviceMethod.postsData) {
        ...
        if (encodingError) {
            NSLog(@"encoding error:%@", [self messageFromError:encodingError]);
        }
    ...
    //
    _downloadTask.priority = _serviceMethod.priority;
    //
...
-(id) parseObjectFromData:(NSData*) data serviceContext:(DCServiceContext**) serviceContext {
    ...
    if (error) {
        ...
        NSLog(@"decoding error:%@", [self messageFromError:error]);
...
-(NSString*) messageFromError:(NSError*) error {
    return [[[error userInfo] objectForKey:NSUnderlyingErrorKey] localizedDescription];
}
